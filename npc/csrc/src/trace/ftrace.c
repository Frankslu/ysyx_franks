...
__attribute_maybe_unused__ static Func_t *func;
__attribute_maybe_unused__ static Func_array_t *func_array = NULL;
__attribute_maybe_unused__ static Fring_t fring[FRING_SIZE] = {};
__attribute_maybe_unused__ static int fring_pos = 0;
	...
	for (Func_array_t *p = func_array; p != NULL; p = p->next){
		...
		for (int i = 0; i < p->cnt; i++){
			if (next_pc == func[i].addr){
				...
				strcpy(fring[fring_pos].filename, p->filename);
				fring_pos = fring_pos == FRING_SIZE - 1 ? 0 : fring_pos + 1;
				log_write("Ftrace: PC %08x call %s %s\t%08x\n", pc, p->filename, func[i].name, next_pc);
				return;
			}
		}
	}
	...
	for (Func_array_t *p = func_array; p != NULL; p = p->next){
		...
		for (int i = 0; i < p->cnt; i++){
			if (next_pc > func[i].addr && next_pc < func[i].addr + func[i].size){
				...
				strcpy(fring[fring_pos].filename, p->filename);
				fring_pos = fring_pos == FRING_SIZE - 1 ? 0 : fring_pos + 1;
				log_write("Ftrace: PC %08x ret  %s %s\t%08x\n", pc, p->filename, func[i].name, next_pc);
				return;
			}
		}
	}
	...
	for (int i=0; i < FRING_SIZE; i++){
		if (fring[pos].func_name[0] != '\0'){
			if (fring[pos].dir == CALL){
				printf("Ftrace: PC %08x call %s %s\t%08x\n", fring[pos].pc, fring[pos].filename, fring[pos].func_name, fring[pos].next_pc);
			}
			else {
				printf("Ftrace: PC %08x ret  %s %s\t%08x\n", fring[pos].pc, fring[pos].filename, fring[pos].func_name, fring[pos].next_pc);
			}
		}
