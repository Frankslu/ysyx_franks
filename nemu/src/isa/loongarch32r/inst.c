...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT_START();
  INSTPAT("010101 ???????????????? ??????????"    , bl       , OFFS26, s->dnpc = s->pc + (int32_t)imm, R(1) = s->snpc, printf("bl, s->pc=%08x, imm=%08x, s->dnpc=%08x\n",s->pc,imm,s->dnpc));
  ...
  INSTPAT("0000001010 ???????????? ????? ?????"   , addi.w   , 2RI12 , R(rd) = imm + src1, printf("addi.w, src1=%08x, src2=%08x, R(rd)=%08x\n", src1, src2, R(rd)));
