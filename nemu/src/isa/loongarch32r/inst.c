...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
	...
	INSTPAT("00000000000111000 ????? ????? ?????"   , mul.w    , 3R     , mul = (int64_t)src1 * (int64_t)src2, R(rd) = (int32_t)mul);
	INSTPAT("00000000000111001 ????? ????? ?????"   , mulh.w   , 3R     , mul = (int64_t)src1 * (int64_t)src2, R(rd) = (int32_t)(mul >> 32));
	INSTPAT("00000000000111010 ????? ????? ?????"   , mulh.wu  , 3R     , mul_u = (uint64_t)src1 * (uint64_t)src2, R(rd) = (uint32_t)(mul_u >> 32));
