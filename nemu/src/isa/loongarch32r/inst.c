...
static inline void decode_operand(Decode *s, int *rj, int *rk, int *rd_, word_t *src1, word_t *src2, word_t *imm, int type) {
...
#define R3 15
	...
	INSTPAT(0b010100, I26, b, OFFS26, s->dnpc = s->pc + imm
			IFDEF(CONFIG_loongarch32r_ITRACE, , sprintf(as, "b\t%d(0x%x) # %x", simm, imm, s->dnpc)));

	INSTPAT(0b010101, I26, bl, OFFS26, s->dnpc = s->pc + imm, R(1) = s->snpc
	...
	INSTPAT(0b010011, I16_2R, ret, 2RO16, s->dnpc = src1 + imm, R(rd) = s->snpc
	...
	INSTPAT(0b010011, I16_2R, jirl, 2RO16, s->dnpc = src1 + imm, R(rd) = s->snpc
	...
	INSTPAT(0b010110, I16_2R, beq, 2RO16, s->dnpc = s->pc + (src1 == R(rd) ? imm : 4)
			IFDEF(CONFIG_loongarch32r_ITRACE, , br_sprintf("beq")));

	INSTPAT(0b010111, I16_2R, bne, 2RO16, s->dnpc = s->pc + (src1 != R(rd) ? imm : 4)
			IFDEF(CONFIG_loongarch32r_ITRACE, , br_sprintf("bne")));

	INSTPAT(0b011000, I16_2R, blt, 2RO16, s->dnpc = s->pc + ((signed)src1 < (signed)R(rd) ? imm : 4)
			IFDEF(CONFIG_loongarch32r_ITRACE, , br_sprintf("blt")));

	INSTPAT(0b011001, I16_2R, bge, 2RO16, s->dnpc = s->pc + ((signed)src1 >= (signed)R(rd) ? imm : 4)
			IFDEF(CONFIG_loongarch32r_ITRACE, , br_sprintf("bge")));

	INSTPAT(0b011010, I16_2R, bltu, 2RO16, s->dnpc = s->pc + ((unsigned)src1 < (unsigned)R(rd) ? imm : 4)
			IFDEF(CONFIG_loongarch32r_ITRACE, , ubr_sprintf("bltu")));

	INSTPAT(0b011011, I16_2R, bgeu, 2RO16, s->dnpc = s->pc + ((unsigned)src1 >= (unsigned)R(rd) ? imm : 4)
			IFDEF(CONFIG_loongarch32r_ITRACE, , ubr_sprintf("bgeu")));

	INSTPAT(0b0001010, I20_R, lu12i.w, 1RI20, R(rd) = imm
			IFDEF(CONFIG_loongarch32r_ITRACE, , R1I20_sp("lu12i.w")));

	INSTPAT(0b0001110, I20_R, pcaddu12i, 1RI20, R(rd) = s->pc + imm
			IFDEF(CONFIG_loongarch32r_ITRACE, , R1I20_sp("pcaddu12i.w")));

	INSTPAT(0b0000001000, I12_2R, slti, 2RI12, R(rd) = (signed)src1 < (signed)imm ? 1 : 0
			IFDEF(CONFIG_loongarch32r_ITRACE, , slti_sp("slti")));

	INSTPAT(0b0000001001, I12_2R, sltui, 2RI12, R(rd) = (unsigned)src1 < (unsigned)imm ? 1 : 0
			IFDEF(CONFIG_loongarch32r_ITRACE, , addi_sp("sltui")));

	INSTPAT(0b0000001010, I12_2R, addi.w, 2RI12, R(rd) = imm + src1
			IFDEF(CONFIG_loongarch32r_ITRACE, , addi_sp("addi.w")));

	INSTPAT(0b0000001101, I12_2R, andi, 2RI12U, R(rd) = imm & src1
			IFDEF(CONFIG_loongarch32r_ITRACE, , addi_sp("andi")));

	INSTPAT(0b0000001110, I12_2R, ori, 2RI12U, R(rd) = imm | src1
			IFDEF(CONFIG_loongarch32r_ITRACE, , addi_sp("ori")));

	INSTPAT(0b0000001111, I12_2R, xori, 2RI12U, R(rd) = imm ^ src1
			IFDEF(CONFIG_loongarch32r_ITRACE, , addi_sp("xori")));

	INSTPAT(0b0010100000, I12_2R, ld.b, 2RI12, R(rd) = (int8_t)Mr(src1 + imm, 1)
			IFDEF(CONFIG_loongarch32r_ITRACE, , ls_sp("ld.b")));

	INSTPAT(0b0010100001, I12_2R, ld.h, 2RI12, R(rd) = (int16_t)Mr(src1 + imm, 2)
			IFDEF(CONFIG_loongarch32r_ITRACE, , ls_sp("ld.h")));

	INSTPAT(0b0010100010, I12_2R, ld.w, 2RI12, R(rd) = Mr(src1 + imm, 4)
			IFDEF(CONFIG_loongarch32r_ITRACE, , ls_sp("ld.w")));

	INSTPAT(0b0010100100, I12_2R, st.b, 2RI12, Mw(src1 + imm, 1, R(rd))
			IFDEF(CONFIG_loongarch32r_ITRACE, , ls_sp("st.b")));

	INSTPAT(0b0010100101, I12_2R, st.h, 2RI12, Mw(src1 + imm, 2, R(rd))
			IFDEF(CONFIG_loongarch32r_ITRACE, , ls_sp("st.h")));

	INSTPAT(0b0010100110, I12_2R, st.w, 2RI12, Mw(src1 + imm, 4, R(rd))
			IFDEF(CONFIG_loongarch32r_ITRACE, , ls_sp("st.w")));

	INSTPAT(0b0010101000, I12_2R, ld.bu, 2RI12, R(rd) = (uint8_t)Mr(src1 + imm, 1)
			IFDEF(CONFIG_loongarch32r_ITRACE, , ls_sp("ld.bu")));

	INSTPAT(0b0010101001, I12_2R, ld.hu, 2RI12, R(rd) = (uint16_t)Mr(src1 + imm, 2)
			IFDEF(CONFIG_loongarch32r_ITRACE, , ls_sp("ld.hu")));

	INSTPAT(0b00000000000100000, R3, add.w, 3R, R(rd) = src1 + src2
			IFDEF(CONFIG_loongarch32r_ITRACE, , add_sp("add.w")));

	INSTPAT(0b00000000000100010, R3, sub.w, 3R, R(rd) = src1 - src2
			IFDEF(CONFIG_loongarch32r_ITRACE, , add_sp("sub.w")));

	INSTPAT(0b00000000000100100, R3, slt, 3R, R(rd) = (signed)src1 < (signed)src2 ? 1 : 0
			IFDEF(CONFIG_loongarch32r_ITRACE, , slt_sp("slt")));

	INSTPAT(0b00000000000100101, R3, sltu, 3R, R(rd) = (unsigned)src1 < (unsigned)src2 ? 1 : 0
			IFDEF(CONFIG_loongarch32r_ITRACE, , add_sp("sltu")));

	INSTPAT(0b00000000000101000, R3, nor, 3R, R(rd) = ~(src1 | src2)
			IFDEF(CONFIG_loongarch32r_ITRACE, , add_sp("nor")));

	INSTPAT(0b00000000000101001, R3, and, 3R, R(rd) = src1 & src2
			IFDEF(CONFIG_loongarch32r_ITRACE, , add_sp("and")));

	INSTPAT(0b00000000000101010, R3, move(or), 3R, R(rd) = src1 | src2
			IFDEF(CONFIG_loongarch32r_ITRACE, , add_sp("move")));

	INSTPAT(0b00000000000101011, R3, xor, 3R, R(rd) = src1 ^ src2
			IFDEF(CONFIG_loongarch32r_ITRACE, , add_sp("xor")));

	INSTPAT(0b00000000000101110, R3, sll.w, 3R, R(rd) = src1 << (src2 & 0x1f)
			IFDEF(CONFIG_loongarch32r_ITRACE, , add_sp("sll.w")));

	INSTPAT(0b00000000000101111, R3, srl.w, 3R, R(rd) = (unsigned)src1 >> (src2 & 0x1f)
			IFDEF(CONFIG_loongarch32r_ITRACE, , add_sp("srl.w")));

	INSTPAT(0b00000000000110000, R3, sra.w, 3R, R(rd) = (signed)src1 >> (src2 & 0x1f)
			IFDEF(CONFIG_loongarch32r_ITRACE, , add_sp("sra.w")));

	INSTPAT(0b00000000000111000, R3, mul.w, 3R, R(rd) = (int32_t)((int64_t)src1 * (int64_t)src2)
			IFDEF(CONFIG_loongarch32r_ITRACE, , add_sp("mul.w")));

	INSTPAT(0b00000000000111001, R3, mulh.w, 3R, R(rd) = (int32_t)(((int64_t)(signed)src1 * (int64_t)(signed)src2) >> 32)
			IFDEF(CONFIG_loongarch32r_ITRACE, , add_sp("mulh.w")));

	INSTPAT(0b00000000000111010, R3, mulh.wu, 3R, R(rd) = (uint32_t)(((uint64_t)src1 * (uint64_t)src2) >> 32)
			IFDEF(CONFIG_loongarch32r_ITRACE, , add_sp("mulh.wu")));

	INSTPAT(0b00000000001000000, R3, div.w, 3R, R(rd) = (signed)src1 / (signed)src2
			IFDEF(CONFIG_loongarch32r_ITRACE, , add_sp("div.w")));

	INSTPAT(0b00000000001000001, R3, mod.w, 3R, R(rd) = (signed)src1 % (signed)src2
			IFDEF(CONFIG_loongarch32r_ITRACE, , add_sp("mod.w")));

	INSTPAT(0b00000000001000010, R3, div.wu, 3R, R(rd) = (unsigned)src1 / (unsigned)src2
			IFDEF(CONFIG_loongarch32r_ITRACE, , add_sp("div.wu")));

	INSTPAT(0b00000000001000011, R3, mod.wu, 3R, R(rd) = (unsigned)src1 % (unsigned)src2
			IFDEF(CONFIG_loongarch32r_ITRACE, , add_sp("mod.wu")));

	INSTPAT(0b00000000010000001, R3, slli.w, 2RI5, R(rd) = src1 << src2
			IFDEF(CONFIG_loongarch32r_ITRACE, , slli_sp("slli.w")));

	INSTPAT(0b00000000010001001, R3, srli.w, 2RI5, R(rd) = (unsigned)src1 >> src2
			IFDEF(CONFIG_loongarch32r_ITRACE, , slli_sp("srli.w")));

	INSTPAT(0b00000000010010001, R3, srai.w, 2RI5, R(rd) = (signed)src1 >> src2
			IFDEF(CONFIG_loongarch32r_ITRACE, , slli_sp("srai.w")));

	INSTPAT(0b00000000001010100, 15, break, N, MUXDEF(CONFIG_BREAKPOINT, exec_break, NEMUTRAP(s->pc, R(4)))
			, s->dnpc = old_pc
