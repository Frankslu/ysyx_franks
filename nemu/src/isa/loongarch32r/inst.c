...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
	...
	INSTPAT("0000001110 ???????????? ????? ?????"   , or       , 2RI12  , R(rd) = imm | src1);
	INSTPAT("0000001111 ???????????? ????? ?????"   , xor      , 2RI12  , R(rd) = imm ^ src1);
	...
	INSTPAT("00000000000101110 ????? ????? ?????"   , sll.w    , 3R     , R(rd) = src1 << (src2 & 0x31));
	INSTPAT("00000000000101111 ????? ????? ?????"   , srl.w    , 3R     , R(rd) = (unsigned)src1 >> (src2 & 0x31));
	INSTPAT("00000000000110000 ????? ????? ?????"   , sra.w    , 3R     , R(rd) = (signed)src1 >> (src2 & 0x31));
