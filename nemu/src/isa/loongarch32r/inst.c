...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
	...
	INSTPAT("010110 ???????????????? ????? ?????"   , beq      , 2RO16 , s->dnpc = s->pc + (src1 == R(rd) ? imm : 0), printf("beq, src1=%08x, R(rd)=%08x, imm=%08x, s->dnpc=%08x\n", src1, R(rd), imm, s->dnpc));
	INSTPAT("010111 ???????????????? ????? ?????"   , bne      , 2RO16 , s->dnpc = s->pc + (src1 != R(rd) ? imm : 0), printf("bne, src1=%08x, R(rd)=%08x, imm=%08x, s->dnpc=%08x\n", src1, R(rd), imm, s->dnpc));
	INSTPAT("011000 ???????????????? ????? ?????"   , blt      , 2RO16 , s->dnpc = s->pc + ((signed)src1 < (signed)R(rd) ? imm : 0), printf("blt, src1=%08x, R(rd)=%08x, imm=%08x, s->dnpc=%08x\n", src1, R(rd), imm, s->dnpc));
	INSTPAT("011001 ???????????????? ????? ?????"   , bge      , 2RO16 , s->dnpc = s->pc + ((signed)src1 >= (signed)R(rd) ? imm : 0), printf("bge, src1=%08x, R(rd)=%08x, imm=%08x, s->dnpc=%08x\n", src1, R(rd), imm, s->dnpc));
	INSTPAT("011010 ???????????????? ????? ?????"   , bltu     , 2RO16 , s->dnpc = s->pc + ((unsigned)src1 < (unsigned)R(rd) ? imm : 0), printf("bltu, src1=%08x, R(rd)=%08x, imm=%08x, s->dnpc=%08x\n", src1, R(rd), imm, s->dnpc));
	INSTPAT("011011 ???????????????? ????? ?????"   , bgeu     , 2RO16 , s->dnpc = s->pc + ((unsigned)src1 >= (unsigned)R(rd) ? imm : 0), printf("bgeu, src1=%08x, R(rd)=%08x, imm=%08x, s->dnpc=%08x\n", src1, R(rd), imm, s->dnpc));
	...
	INSTPAT("0000001000 ???????????? ????? ?????"   , slti     , 2RI12 , R(rd) = (signed)src1 < (signed)imm ? 1 : 0, printf("addi.w, src1=%08x, imm=%08x, R(rd)=%08x\n", src1, imm, R(rd)));
	INSTPAT("0000001001 ???????????? ????? ?????"   , sltui    , 2RI12 , R(rd) = (unsigned)src1 < (unsigned)imm ? 1 : 0, printf("addi.w, src1=%08x, imm=%08x, R(rd)=%08x\n", src1, imm, R(rd)));
